<template>
  <div class="city_container">
    <head-top :title="cityname" go-back='true'>
...
<script>
  import headTop from '../components/layout/Header'
  import {getCityInfo} from '../server/getCityData'
  import {getSearchData} from '../server/serach'
  import {getLocalStore, saveStore, delStore} from '../config/units'
  ...
  export default {
    ...
    methods:{
      ...
      postpois(){
        //输入值不为空时才发送信息
        if (this.inputVaule) {
          getSearchData(this.id, this.inputVaule).then(res => {
      ...
      nextpage(index, geohash){
        let history = getLocalStore('placeHistory');
        ...
        saveStore('placeHistory',this.placeHistory)
        this.$router.push({path:'/meta', query:{geohash}})
      },
      clearAll(){
